...
function PersistentConnectionLine({
  fromId,
  toId,
}: {
  fromId: string;
  ...
  useFrame((state) => {
    if (lineRef.current) {
      ...
      if (lineRef.current.material) {
        ...
        if (isActive) {
          // Active shimmer — pure white, fully bright
          const shimmer = Math.sin(state.clock.elapsedTime * 15.0) * 0.5 + 0.5;
          material.opacity = 0.9 + shimmer * 0.1;
          material.color.setHex(0xffffff);

          if (material.uniforms) {
            if (material.uniforms.opacity)
              material.uniforms.opacity.value = 0.9 + shimmer * 0.1;
            if (material.uniforms.color)
              material.uniforms.color.value.setHex(0xffffff);
            if (material.uniforms.lineWidth)
              material.uniforms.lineWidth.value = 2.5;
          }
        } else {
          // Persistent connection — pure white, steady
          material.color.setHex(0xffffff);
          const pulse = Math.sin(state.clock.elapsedTime * 2.0) * 0.05 + 0.75;
          material.opacity = pulse;

          if (material.uniforms) {
            if (material.uniforms.opacity)
              material.uniforms.opacity.value = pulse;
            if (material.uniforms.color)
              material.uniforms.color.value.setHex(0xffffff);
            if (material.uniforms.lineWidth)
              material.uniforms.lineWidth.value = 1.5;
          }
        }
      }
    }
...
function RelayLine({
  ...
  useFrame((state) => {
    if (lineRef.current) {
      ...
      if (flatPoints.length >= 6) {
        // Need at least 2 points (x,y,z each)
      ...
      if (lineRef.current.material) {
        ...
        if (isActive) {
          // Shimmer effect — pure white, maximum brightness
          const shimmer = Math.sin(state.clock.elapsedTime * 15.0) * 0.5 + 0.5;
          material.opacity = 0.9 + shimmer * 0.1;
          material.color.setHex(0xffffff);
        } else {
  ...
  const lineWidth = isSelected ? 2.5 : isActive ? 3.5 : 2.0;
      ...
      if (lineRef.current.material) {
        ...
        if (material.uniforms) {
          if (material.uniforms.opacity)
            material.uniforms.opacity.value = pulse;
          if (material.uniforms.color)
            material.uniforms.color.value.setHex(0x0088ff);
        }
      }
    }
  });
...
function DynamicRelayConnection({ index }: { index: number }) {
  ...
  useFrame((state) => {
    ...
    const store = useStore.getState();
    const relayDrones = store.drones.filter(
      (d) => d.role === "relay" && d.status === "online",
    );
    const otherDrones = store.drones.filter(
      (d) => d.role !== "relay" && d.status === "online",
    );
